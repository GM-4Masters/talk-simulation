...
public class ChatUI : MonoBehaviour
{
    ...
    private IEnumerator ChatCrt()
    {
        ...
        for (int i=0; i<=currentChatIndex; i++)
        {
            ...
            if (chatroom.Equals(Constants.grouptalk))
            {
                if (DataManager.Instance.goodChoiceList[episodeIndex].Count-1 > passedChoiceNum && 
                    (DataManager.Instance.goodChoiceList[episodeIndex][passedChoiceNum + 1][0].Equals(currentChatData.index) ||
                    DataManager.Instance.badChoiceList[episodeIndex][passedChoiceNum + 1][0].Equals(currentChatData.index)))
                    passedChoiceNum++;
            }
